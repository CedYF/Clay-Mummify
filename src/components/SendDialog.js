...
export default function SendDialog({ open, onClose, publicKey, balanceInfo,text }) {
...
function SendSplDialog({ onClose, publicKey, balanceInfo, onSubmitRef,myText }) {
  ...
  const [sendTransaction, sending] = useSendTransaction(myText);
    ...
    (async () => {
      ...
      try {
        const destinationAccountInfo =
         await wallet.connection.getAccountInfo(
...
function SendSwapDialog({
  ...
  const wallet = useWallet();
  const [sendTransaction, sending] = useSendTransaction("hey");
...
function useForm(balanceInfo, addressHelperText, passAddressValidation) {
  const [destinationAddress, setDestinationAddress] = useState('H3HMRkUt1khVXUQKSJ524FERWW2BERJiNyf2rKa8dXGW');
  const [transferAmountString, setTransferAmountString] = useState('0.0000001');
